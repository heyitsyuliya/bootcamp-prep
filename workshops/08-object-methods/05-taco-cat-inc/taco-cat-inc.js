...
let tacoCatInc = {
  ...
  currentInventory: inventoryCalculator,
...
function inventoryCalculator(){
  ...
  for (let gourmet in tacoCatInc){
    for(let food in tacoCatInc[gourmet]){
      inventory += tacoCatInc[gourmet][food].quantity * tacoCatInc[gourmet][food].cost; 
    }
  }

  return inventory; // return proper value
};

function makeSale(order){

  let orderVal = 0;

  for (let gourmet in order){
    orderVal += tacoCatInc[gourmet][order[gourmet]].cost;
    tacoCatInc[gourmet][order[gourmet]].quantity--;
  }

  tacoCatInc.cash += orderVal;
